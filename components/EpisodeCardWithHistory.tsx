...
interface EpisodeCardWithHistoryProps {
    episode: Episode;
}

export default function EpisodeCardWithHistory({ episode }: EpisodeCardWithHistoryProps) {
    const { isRead } = useReadingHistory();
    const read = isRead(episode.slug);
        ...
        <article className="group h-full flex flex-col pt-3 border-t border-gray-300 hover:border-accent transition-colors relative">
            ...
            <div className="flex items-center justify-between mb-1.5 relative z-10">
                ...
                <div className="flex items-center gap-1.5">
                    {read && (
                        <span className="text-[9px] font-sans text-gray-400 flex items-center gap-0.5">
            ...
            <Link href={`/episode/${episode.slug}`} className="flex flex-col flex-grow group-hover:text-accent transition-colors">
                {/* Headline */}
                <h2 className={`text-lg font-bold leading-tight group-hover:text-accent transition-colors mb-2 ${read ? 'text-gray-500' : 'text-gray-900'}`}>
                    {episode.title}
                </h2>
